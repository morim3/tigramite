...
class PCMCIParallel(PCMCIbase):
    ...
    * false discovery control and confidence interval estimation
    ...
    @staticmethod
    def _iter_conditions(parent, conds_dim, all_parents):
    ...
    @staticmethod
    def _sort_parents(parents_vals):
    ...
    @staticmethod
    def _set_max_condition_dim(max_conds_dim, tau_min, tau_max, N):
        """
        ...
        if max_conds_dim is None:
            max_conds_dim = N * (tau_max - tau_min + 1)
    ...
    def run_pc_stable(self,
        ...
        results = joblib.Parallel(n_jobs=self.n_parallel)([joblib.delayed(self._run_pc_stable_single)(
            ...
            max_combinations=max_combinations) for j in range(self.N)])

        for j in range(self.N):
            all_parents[j] = results[j]['parents']
            val_min[j] = results[j]['val_min']
            pval_max[j] = results[j]['pval_max']
            iterations[j] = results[j]['iterations']
