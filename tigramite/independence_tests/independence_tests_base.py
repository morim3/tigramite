...
class CondIndTestAtomic():
    ...
    @staticmethod
    def _get_array(measure, dataframe, mask_type, X, Y, Z, tau_max=0, cut_off='2xtau_max', 
                   remove_constant_data=False,
                   verbosity=0):
        """Convencience wrapper around construct_array."""

        if measure in ['par_corr', 'par_corr_wls', 'robust_par_corr', 'regressionCI', 
                            'gsquared', 'gp_dc']:
            if len(X) > 1 or len(Y) > 1:
                raise ValueError("X and Y for %s must be univariate." %
                                        measure)
        # Call the wrapped function
        array, xyz, XYZ, type_array = dataframe.construct_array(X=X, Y=Y, Z=Z,
                                              tau_max=tau_max,
                                              mask_type=mask_type,
    ...
    def run_test(self, X, Y, Z=None, tau_max=0, cut_off='2xtau_max', alpha_or_thres=None):
        ...
        (array, xyz, XYZ, data_type, 
         nonzero_array, nonzero_xyz, nonzero_XYZ, nonzero_data_type) = CondIndTestAtomic._get_array(
                                            measure=self.measure, dataframe=self.dataframe, mask_type=self.mask_type,
        ...
        if self.verbosity > 1:
            self._print_cond_ind_results(val=val, pval=pval, dependent=dependent,
                                         conf=None)
